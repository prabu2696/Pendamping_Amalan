// remedial_core.js - Core logic for Remedial & Pengayaan
window.RemedialCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('g_remedial'.startsWith('k_')) {
      ...
      } else if ('g_remedial'.startsWith('t_')) {
      ...
      } else if ('g_remedial'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[RemedialCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_g_remedial`;
