// kokurikuler_ui.js - UI rendering for Modul Kokurikuler
window.KokurikulerUI = {
  schema: [
  {
    "type": "select",
  ...
  {
    "type": "select",
    "label": "Dimensi Profil Pancasila",
    "options": [
      "Beriman & Bertakwa",
  ...
  {
    "type": "textarea",
    "label": "Deskripsi Rencana Projek",
    "placeholder": "Jelaskan aktivitas utama dan luaran (output) proyek...",
    "required": true
  }
],
  container: null,
  ...
  renderForm(containerId) {
      ...
      <div class="cyber-container fade-in" style="height:100%; display:flex; flex-direction:column; padding:10px;">
         ...
         <div style="flex:1; overflow-y:auto; padding-right:15px;" class="custom-scrollbar">
             ...
             <div class="cyber-group" style="margin-top:30px; grid-column: 1 / -1;">
                ...
                <textarea id="prompt_tuning_g_kokurikuler" class="cyber-input" placeholder="Berikan instruksi kustom kepada AI. Contoh: Buat gaya bahasa santai namun formal, sertakan tabel penilaian di akhir dokumen..." style="background:rgba(255,0,255,0.02); border-color:rgba(255,0,255,0.2)"></textarea>
             </div>
         </div>
         ...
         <div style="padding-top:20px; border-top:1px solid rgba(0,229,255,0.15); margin-top:15px; display:flex; gap:15px; justify-content:flex-end;">
             <button class="cyber-button" onclick="window.KokurikulerUI.saveDraft()">
                 <span style="opacity:0.8">💾</span> Simpan Draft
             </button>
             <button class="cyber-button primary" style="box-shadow: 0 0 20px rgba(0,229,255,0.3); padding: 12px 24px; font-size:13px;" onclick="window.KokurikulerUI.executeAI()">
    ...
    window.KokurikulerCore.loadDraft().then(draft => {
      if (draft && draft.payloadData) {
        ...
        const pTuning = document.getElementById('prompt_tuning_g_kokurikuler');
    ...
    }).catch(e => console.warn('[KokurikulerUI] Failed to load draft:', e));
  },
  ...
  gatherInputs() {
    ...
    const userPrompt = document.getElementById('prompt_tuning_g_kokurikuler')?.value.trim() || '';
  ...
  async saveDraft() {
    ...
    try {
      await window.KokurikulerCore.saveDraft(payloadData, userPrompt);
  ...
  async executeAI() {
    ...
    window.KokurikulerCore.executeAI(
      payloadData,
      userPrompt,
      () => {},
      (text) => {
  ...
  renderResult(text, formHtmlBackup, payloadData) {
    ...
    const exportButtonHtml = isExcelModule
      ? `<button class="cyber-button" onclick="window.KokurikulerUI.exportExcel()">
           <span>📊</span> Ekspor Excel (.xls)
         </button>`
      : `<button class="cyber-button" onclick="window.KokurikulerUI.exportWord()">
    ...
    this.container.innerHTML = `
      <div class="cyber-container fade-in" style="height:100%; display:flex; flex-direction:column; padding:10px;">
         ...
         <div style="padding-top:20px; border-top:1px solid rgba(0,229,255,0.15); display:flex; gap:15px; justify-content:space-between; align-items:center;">
             <button class="cyber-button" onclick="window.KokurikulerUI.restoreForm()">
  ...
  updatePreview() {
    ...
    const schoolProfile = window.KokurikulerCore.schoolProfile;
    const kop = `
  ...
  async saveFinalDocument() {
    ...
    if (!editor) return;
    try {
      await window.KokurikulerCore.saveFinalDocument(editor.value);
