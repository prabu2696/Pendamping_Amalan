// buku_nilai_core.js - Core logic for Buku Nilai Digital
window.BukuNilaiCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('g_buku_nilai'.startsWith('k_')) {
      ...
      } else if ('g_buku_nilai'.startsWith('t_')) {
      ...
      } else if ('g_buku_nilai'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[BukuNilaiCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_g_buku_nilai`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'g_buku_nilai',
      title: 'Buku Nilai Digital',
