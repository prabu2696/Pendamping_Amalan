// kokurikuler_core.js - Core logic for Modul Kokurikuler
window.KokurikulerCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      let systemPrompt = '';
      if ('g_kokurikuler'.startsWith('g_')) {
      ...
      } else if ('g_kokurikuler'.startsWith('k_')) {
      ...
      } else if ('g_kokurikuler'.startsWith('t_')) {
      ...
      } else if ('g_kokurikuler'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[KokurikulerCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_g_kokurikuler`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'g_kokurikuler',
      title: 'Modul Kokurikuler',
