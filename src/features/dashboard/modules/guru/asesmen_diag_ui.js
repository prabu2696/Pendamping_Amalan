// asesmen_diag_ui.js - UI rendering for Asesmen Diagnostik
window.AsesmenDiagUI = {
  schema: [
  {
    "type": "select",
  ...
  {
    "type": "select",
    "label": "Tipe Diagnostik",
    "options": [
      "Kognitif (Kemampuan Awal)",
      "Non-Kognitif (Gaya Belajar & Minat)",
      "Kombinasi"
    ],
    "required": true
  },
  {
    "type": "textarea",
    "label": "Materi Prasyarat / Aspek Dinilai",
    "placeholder": "Contoh: Kemampuan berhitung 1-100, gaya belajar anak di rumah...",
    "required": true
  }
],
  container: null,
  ...
  renderForm(containerId) {
    ...
    const fieldsHtml = this.schema.map((field, idx) => {
      const fieldId = `field_g_asesmen_diag_${idx}`;
      let inputHtml = '';
      ...
      <div class="cyber-container fade-in" style="height:100%; display:flex; flex-direction:column; padding:10px;">
         <div class="cyber-title" style="display:flex; justify-content:space-between; align-items:center; border-bottom: 2px solid rgba(0,229,255,0.2); padding-bottom: 15px; margin-bottom: 20px;">
             <div style="display:flex; align-items:center; gap:12px;">
                 ...
                 <div style="display:flex; flex-direction:column;">
                     <span style="font-size:18px; font-weight:700; letter-spacing:1px; color:#fff; text-shadow: 0 0 10px rgba(0,229,255,0.3); font-family:'Plus Jakarta Sans', sans-serif;">${'Asesmen Diagnostik'}</span>
                     <span style="font-size:11px; color:rgba(0,229,255,0.8); text-transform:uppercase; letter-spacing:2px; font-family:'Plus Jakarta Sans', sans-serif;">${'Asesmen & Penilaian'}</span>
                 </div>
             </div>
         ...
         <div style="flex:1; overflow-y:auto; padding-right:15px;" class="custom-scrollbar">
             <div style="background:rgba(255,255,255,0.02); border-left:3px solid #00e5ff; padding:12px 15px; border-radius:0 8px 8px 0; color:#a0c0d0; font-size:12px; margin-bottom:25px; line-height:1.6; font-family:'Plus Jakarta Sans', sans-serif;">
                 ${'Kognitif & Non-Kognitif'}. Isi formulir parameter di bawah ini. Semua field yang memiliki label menyala (*) wajib diisi sebelum eksekusi AI Deep Learning Cimega dimulai.
             </div>
             ...
             <div class="cyber-group" style="margin-top:30px; grid-column: 1 / -1;">
                ...
                <textarea id="prompt_tuning_g_asesmen_diag" class="cyber-input" placeholder="Berikan instruksi kustom kepada AI. Contoh: Buat gaya bahasa santai namun formal, sertakan tabel penilaian di akhir dokumen..." style="background:rgba(255,0,255,0.02); border-color:rgba(255,0,255,0.2)"></textarea>
             </div>
         </div>
         ...
         <div style="padding-top:20px; border-top:1px solid rgba(0,229,255,0.15); margin-top:15px; display:flex; gap:15px; justify-content:flex-end;">
             <button class="cyber-button" onclick="window.AsesmenDiagUI.saveDraft()">
                 <span style="opacity:0.8">💾</span> Simpan Draft
             </button>
             <button class="cyber-button primary" style="box-shadow: 0 0 20px rgba(0,229,255,0.3); padding: 12px 24px; font-size:13px;" onclick="window.AsesmenDiagUI.executeAI()">
    ...
    // Load draft otomatis
    window.AsesmenDiagCore.loadDraft().then(draft => {
      if (draft && draft.payloadData) {
        this.schema.forEach((field, idx) => {
          const el = document.getElementById(`field_g_asesmen_diag_${idx}`);
          if (el && draft.payloadData[field.label] !== undefined) {
            el.value = draft.payloadData[field.label];
          }
        });
        const pTuning = document.getElementById('prompt_tuning_g_asesmen_diag');
    ...
    }).catch(e => console.warn('[AsesmenDiagUI] Failed to load draft:', e));
  },
  ...
  gatherInputs() {
    ...
    const userPrompt = document.getElementById('prompt_tuning_g_asesmen_diag')?.value.trim() || '';
  ...
  async saveDraft() {
    ...
    try {
      await window.AsesmenDiagCore.saveDraft(payloadData, userPrompt);
  ...
  async executeAI() {
    ...
    window.AsesmenDiagCore.executeAI(
      payloadData,
      userPrompt,
      () => {},
      (text) => {
  ...
  renderResult(text, formHtmlBackup, payloadData) {
    ...
    const exportButtonHtml = isExcelModule
      ? `<button class="cyber-button" onclick="window.AsesmenDiagUI.exportExcel()">
           <span>📊</span> Ekspor Excel (.xls)
         </button>`
      : `<button class="cyber-button" onclick="window.AsesmenDiagUI.exportWord()">
      ...
      <div class="cyber-container fade-in" style="height:100%; display:flex; flex-direction:column; padding:10px;">
         <div class="cyber-title" style="display:flex; justify-content:space-between; align-items:center; border-bottom: 2px solid rgba(0,229,255,0.2); padding-bottom: 15px; margin-bottom: 20px;">
             <div style="display:flex; align-items:center; gap:12px;">
                 <div style="font-size:24px; background:rgba(0,229,255,0.1); width:45px; height:45px; display:flex; align-items:center; justify-content:center; border-radius:10px; border:1px solid rgba(0,229,255,0.3); box-shadow: 0 0 15px rgba(0,229,255,0.1)">✨</div>
                 <div style="display:flex; flex-direction:column;">
                     <span style="font-size:18px; font-weight:700; letter-spacing:1px; color:#fff; text-shadow: 0 0 10px rgba(0,229,255,0.3); font-family:'Plus Jakarta Sans', sans-serif;">PREVIEW EDITOR: ${'Asesmen Diagnostik'}</span>
         ...
         <div style="flex:1; display:flex; gap:20px; overflow:hidden; margin-bottom:15px;">
            <div style="flex:1; display:flex; flex-direction:column; background:rgba(255,255,255,0.01); border:1px solid rgba(0,229,255,0.15); border-radius:8px; padding:12px;">
               ...
               <textarea id="ai-result-editor" class="cyber-input" style="flex:1; width:100%; height:100%; font-family: 'Courier New', Courier, monospace; font-size:13px; line-height:1.6; resize:none; background:rgba(0,0,0,0.35); border-color:rgba(0,229,255,0.2); color:#e0f0ff;" oninput="window.AsesmenDiagUI.updatePreview()">${text.trim()}</textarea>
            </div>
         ...
         <div style="padding-top:20px; border-top:1px solid rgba(0,229,255,0.15); display:flex; gap:15px; justify-content:space-between; align-items:center;">
             <button class="cyber-button" onclick="window.AsesmenDiagUI.restoreForm()">
                 <span>←</span> Edit Parameter
             </button>
             <div style="display:flex; gap:10px;">
                 <button class="cyber-button" onclick="window.AsesmenDiagUI.saveFinalDocument()">
                     <span>💾</span> Simpan Dokumen
                 </button>
                 ${exportButtonHtml}
                 <button class="cyber-button primary" onclick="window.AsesmenDiagUI.printDocument()">
  ...
  updatePreview() {
    ...
    const schoolProfile = window.AsesmenDiagCore.schoolProfile;
    const kop = `
  ...
  async saveFinalDocument() {
    ...
    if (!editor) return;
    try {
      await window.AsesmenDiagCore.saveFinalDocument(editor.value);
  ...
  async exportWord() {
    ...
    try {
      ...
      `;
      const cleanFileName = 'Asesmen_Diagnostik';
