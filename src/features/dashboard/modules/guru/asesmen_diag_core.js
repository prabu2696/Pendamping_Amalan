// asesmen_diag_core.js - Core logic for Asesmen Diagnostik
window.AsesmenDiagCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      let systemPrompt = '';
      if ('g_asesmen_diag'.startsWith('g_')) {
      ...
      } else if ('g_asesmen_diag'.startsWith('k_')) {
      ...
      } else if ('g_asesmen_diag'.startsWith('t_')) {
      ...
      } else if ('g_asesmen_diag'.startsWith('b_')) {
...
DOKUMEN: Asesmen Diagnostik
KATEGORI: Asesmen & Penilaian
IDENTITAS SEKOLAH:
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[AsesmenDiagCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_g_asesmen_diag`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'g_asesmen_diag',
