// cp_tp_atp_core.js - Core logic for Pemetaan CP, TP, ATP
window.CpTpAtpCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('g_cp_tp_atp'.startsWith('k_')) {
      ...
      } else if ('g_cp_tp_atp'.startsWith('t_')) {
      ...
      } else if ('g_cp_tp_atp'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[CpTpAtpCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_g_cp_tp_atp`;
