// kisi_soal_core.js - Core logic for Kisi-Kisi & Analisis Soal
window.KisiSoalCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('g_kisi_soal'.startsWith('k_')) {
      ...
      } else if ('g_kisi_soal'.startsWith('t_')) {
      ...
      } else if ('g_kisi_soal'.startsWith('b_')) {
...
DOKUMEN: Kisi-Kisi & Analisis Soal
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[KisiSoalCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_g_kisi_soal`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'g_kisi_soal',
      title: 'Kisi-Kisi & Analisis Soal',
