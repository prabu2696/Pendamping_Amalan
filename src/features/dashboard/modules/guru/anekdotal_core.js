// anekdotal_core.js - Core logic for Catatan Anekdotal
window.AnekdotalCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('g_anekdotal'.startsWith('k_')) {
      ...
      } else if ('g_anekdotal'.startsWith('t_')) {
      ...
      } else if ('g_anekdotal'.startsWith('b_')) {
...
DOKUMEN: Catatan Anekdotal
KATEGORI: Harian & Pelaporan
IDENTITAS SEKOLAH:
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[AnekdotalCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_g_anekdotal`;
