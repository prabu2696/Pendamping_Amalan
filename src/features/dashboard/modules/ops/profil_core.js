// profil_core.js - Core logic for Pengaturan Profil Sekolah
window.ProfilCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('o_profil'.startsWith('k_')) {
      ...
      } else if ('o_profil'.startsWith('t_')) {
      ...
      } else if ('o_profil'.startsWith('b_')) {
...
DOKUMEN: Pengaturan Profil Sekolah
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[ProfilCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_o_profil`;
