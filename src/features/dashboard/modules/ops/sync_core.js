// sync_core.js - Core logic for Modul Sinkronisasi Dapodik
window.SyncCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
...
DOKUMEN: Modul Sinkronisasi Dapodik
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[SyncCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_o_sync`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'o_sync',
      title: 'Modul Sinkronisasi Dapodik',
