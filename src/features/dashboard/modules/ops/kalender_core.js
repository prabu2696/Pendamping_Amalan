// kalender_core.js - Core logic for Setup Tahun Ajaran & Kalender
window.KalenderCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('o_kalender'.startsWith('k_')) {
      ...
      } else if ('o_kalender'.startsWith('t_')) {
      ...
      } else if ('o_kalender'.startsWith('b_')) {
...
DOKUMEN: Setup Tahun Ajaran & Kalender
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[KalenderCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_o_kalender`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'o_kalender',
      title: 'Setup Tahun Ajaran & Kalender',
