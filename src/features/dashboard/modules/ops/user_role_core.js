// user_role_core.js - Core logic for Manajemen Pengguna & Role
window.UserRoleCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('o_user_role'.startsWith('k_')) {
      ...
      } else if ('o_user_role'.startsWith('t_')) {
      ...
      } else if ('o_user_role'.startsWith('b_')) {
...
DOKUMEN: Manajemen Pengguna & Role
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[UserRoleCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_o_user_role`;
