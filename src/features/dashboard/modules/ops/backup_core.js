// backup_core.js - Core logic for Database Backup & Recovery
window.BackupCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('o_backup'.startsWith('k_')) {
      ...
      } else if ('o_backup'.startsWith('t_')) {
      ...
      } else if ('o_backup'.startsWith('b_')) {
...
DOKUMEN: Database Backup & Recovery
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[BackupCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_o_backup`;
