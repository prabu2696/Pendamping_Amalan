// bantu_kas_core.js - Core logic for Buku Pembantu Kas & Bank
window.BantuKasCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('b_bantu_kas'.startsWith('k_')) {
      ...
      } else if ('b_bantu_kas'.startsWith('t_')) {
      ...
      } else if ('b_bantu_kas'.startsWith('b_')) {
...
DOKUMEN: Buku Pembantu Kas & Bank
KATEGORI: Pembukuan
IDENTITAS SEKOLAH:
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[BantuKasCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_b_bantu_kas`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'b_bantu_kas',
      title: 'Buku Pembantu Kas & Bank',
