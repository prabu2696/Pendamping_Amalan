// bantu_pajak_core.js - Core logic for Buku Pembantu Pajak
window.BantuPajakCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      let systemPrompt = '';
      if ('b_bantu_pajak'.startsWith('g_')) {
      ...
      } else if ('b_bantu_pajak'.startsWith('k_')) {
      ...
      } else if ('b_bantu_pajak'.startsWith('t_')) {
      ...
      } else if ('b_bantu_pajak'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[BantuPajakCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_b_bantu_pajak`;
