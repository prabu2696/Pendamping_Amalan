// kepegawaian_core.js - Core logic for Administrasi Kepegawaian
window.KepegawaianCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      let systemPrompt = '';
      if ('t_kepegawaian'.startsWith('g_')) {
      ...
      } else if ('t_kepegawaian'.startsWith('k_')) {
      ...
      } else if ('t_kepegawaian'.startsWith('t_')) {
      ...
      } else if ('t_kepegawaian'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[KepegawaianCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_t_kepegawaian`;
