// buku_induk_core.js - Core logic for Buku Induk Siswa
window.BukuIndukCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('t_buku_induk'.startsWith('k_')) {
      ...
      } else if ('t_buku_induk'.startsWith('t_')) {
      ...
      } else if ('t_buku_induk'.startsWith('b_')) {
...
DOKUMEN: Buku Induk Siswa
KATEGORI: Administrasi Siswa
IDENTITAS SEKOLAH:
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[BukuIndukCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_t_buku_induk`;
