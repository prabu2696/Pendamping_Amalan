// ekspedisi_core.js - Core logic for Buku Ekspedisi
window.EkspedisiCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('t_ekspedisi'.startsWith('k_')) {
      ...
      } else if ('t_ekspedisi'.startsWith('t_')) {
      ...
      } else if ('t_ekspedisi'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[EkspedisiCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_t_ekspedisi`;
