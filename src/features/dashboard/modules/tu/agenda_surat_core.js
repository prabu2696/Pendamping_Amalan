// agenda_surat_core.js - Core logic for Agenda Surat Masuk/Keluar
window.AgendaSuratCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      let systemPrompt = '';
      if ('t_agenda_surat'.startsWith('g_')) {
      ...
      } else if ('t_agenda_surat'.startsWith('k_')) {
      ...
      } else if ('t_agenda_surat'.startsWith('t_')) {
      ...
      } else if ('t_agenda_surat'.startsWith('b_')) {
...
DOKUMEN: Agenda Surat Masuk/Keluar
KATEGORI: Administrasi Persuratan
IDENTITAS SEKOLAH:
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[AgendaSuratCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_t_agenda_surat`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 't_agenda_surat',
      title: 'Agenda Surat Masuk/Keluar',
