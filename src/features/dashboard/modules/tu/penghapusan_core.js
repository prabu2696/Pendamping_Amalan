// penghapusan_core.js - Core logic for Buku Penghapusan Barang
window.PenghapusanCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      let systemPrompt = '';
      if ('t_penghapusan'.startsWith('g_')) {
      ...
      } else if ('t_penghapusan'.startsWith('k_')) {
      ...
      } else if ('t_penghapusan'.startsWith('t_')) {
      ...
      } else if ('t_penghapusan'.startsWith('b_')) {
...
DOKUMEN: Buku Penghapusan Barang
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[PenghapusanCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_t_penghapusan`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 't_penghapusan',
      title: 'Buku Penghapusan Barang',
