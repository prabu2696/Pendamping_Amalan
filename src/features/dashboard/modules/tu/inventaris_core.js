// inventaris_core.js - Core logic for Manajemen Inventaris (KIB)
window.InventarisCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('t_inventaris'.startsWith('k_')) {
      ...
      } else if ('t_inventaris'.startsWith('t_')) {
      ...
      } else if ('t_inventaris'.startsWith('b_')) {
...
DOKUMEN: Manajemen Inventaris (KIB)
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[InventarisCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_t_inventaris`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 't_inventaris',
      title: 'Manajemen Inventaris (KIB)',
