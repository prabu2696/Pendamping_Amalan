// gen_sk_core.js - Core logic for Generator SK Bebas
window.GenSkCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('k_gen_sk'.startsWith('k_')) {
      ...
      } else if ('k_gen_sk'.startsWith('t_')) {
      ...
      } else if ('k_gen_sk'.startsWith('b_')) {
...
DOKUMEN: Generator SK Bebas
KATEGORI: Manajerial & Perencanaan
IDENTITAS SEKOLAH:
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[GenSkCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_k_gen_sk`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'k_gen_sk',
      title: 'Generator SK Bebas',
