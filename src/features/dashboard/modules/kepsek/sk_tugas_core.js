// sk_tugas_core.js - Core logic for SK Pembagian Tugas
window.SkTugasCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      } else if ('k_sk_tugas'.startsWith('k_')) {
      ...
      } else if ('k_sk_tugas'.startsWith('t_')) {
      ...
      } else if ('k_sk_tugas'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[SkTugasCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_k_sk_tugas`;
