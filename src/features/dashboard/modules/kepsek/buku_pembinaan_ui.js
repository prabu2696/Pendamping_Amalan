// buku_pembinaan_ui.js - UI rendering for Buku Pembinaan
window.BukuPembinaanUI = {
  schema: [
  {
    "type": "text",
    "label": "Nama Staf / Guru yang Dibina",
    "placeholder": "Contoh: Rina Amalia, A.Ma",
    "required": true
  },
  {
    "type": "date",
    "label": "Tanggal Pembinaan",
    "required": true
  },
  {
    "type": "textarea",
    "label": "Uraian Masalah / Pelanggaran / Kebutuhan",
    "placeholder": "Jelaskan kronologi atau topik pembinaan...",
    "required": true
  },
  {
    "type": "textarea",
    "label": "Tindak Lanjut & Kesepakatan Bersama",
    "placeholder": "Komitmen yang disepakati untuk perbaikan...",
    "required": true
  }
],
  container: null,
  ...
  renderForm(containerId) {
    ...
    const fieldsHtml = this.schema.map((field, idx) => {
      const fieldId = `field_k_buku_pembinaan_${idx}`;
      let inputHtml = '';
      ...
      <div class="cyber-container fade-in" style="height:100%; display:flex; flex-direction:column; padding:10px;">
         <div class="cyber-title" style="display:flex; justify-content:space-between; align-items:center; border-bottom: 2px solid rgba(0,229,255,0.2); padding-bottom: 15px; margin-bottom: 20px;">
             <div style="display:flex; align-items:center; gap:12px;">
                 ...
                 <div style="display:flex; flex-direction:column;">
                     <span style="font-size:18px; font-weight:700; letter-spacing:1px; color:#fff; text-shadow: 0 0 10px rgba(0,229,255,0.3); font-family:'Plus Jakarta Sans', sans-serif;">${'Buku Pembinaan'}</span>
                     <span style="font-size:11px; color:rgba(0,229,255,0.8); text-transform:uppercase; letter-spacing:2px; font-family:'Plus Jakarta Sans', sans-serif;">${'Pengawasan'}</span>
                 </div>
             </div>
         ...
         <div style="flex:1; overflow-y:auto; padding-right:15px;" class="custom-scrollbar">
             <div style="background:rgba(255,255,255,0.02); border-left:3px solid #00e5ff; padding:12px 15px; border-radius:0 8px 8px 0; color:#a0c0d0; font-size:12px; margin-bottom:25px; line-height:1.6; font-family:'Plus Jakarta Sans', sans-serif;">
                 ${'Catatan pembinaan staf'}. Isi formulir parameter di bawah ini. Semua field yang memiliki label menyala (*) wajib diisi sebelum eksekusi AI Deep Learning Cimega dimulai.
             </div>
             ...
             <div class="cyber-group" style="margin-top:30px; grid-column: 1 / -1;">
                ...
                <textarea id="prompt_tuning_k_buku_pembinaan" class="cyber-input" placeholder="Berikan instruksi kustom kepada AI. Contoh: Buat gaya bahasa santai namun formal, sertakan tabel penilaian di akhir dokumen..." style="background:rgba(255,0,255,0.02); border-color:rgba(255,0,255,0.2)"></textarea>
             </div>
         </div>
         ...
         <div style="padding-top:20px; border-top:1px solid rgba(0,229,255,0.15); margin-top:15px; display:flex; gap:15px; justify-content:flex-end;">
             <button class="cyber-button" onclick="window.BukuPembinaanUI.saveDraft()">
                 <span style="opacity:0.8">💾</span> Simpan Draft
             </button>
             <button class="cyber-button primary" style="box-shadow: 0 0 20px rgba(0,229,255,0.3); padding: 12px 24px; font-size:13px;" onclick="window.BukuPembinaanUI.executeAI()">
    ...
    // Load draft otomatis
    window.BukuPembinaanCore.loadDraft().then(draft => {
      if (draft && draft.payloadData) {
        this.schema.forEach((field, idx) => {
          const el = document.getElementById(`field_k_buku_pembinaan_${idx}`);
          if (el && draft.payloadData[field.label] !== undefined) {
            el.value = draft.payloadData[field.label];
          }
        });
        const pTuning = document.getElementById('prompt_tuning_k_buku_pembinaan');
    ...
    }).catch(e => console.warn('[BukuPembinaanUI] Failed to load draft:', e));
  },
  ...
  gatherInputs() {
    ...
    const userPrompt = document.getElementById('prompt_tuning_k_buku_pembinaan')?.value.trim() || '';
  ...
  async saveDraft() {
    ...
    try {
      await window.BukuPembinaanCore.saveDraft(payloadData, userPrompt);
  ...
  async executeAI() {
    ...
    window.BukuPembinaanCore.executeAI(
      payloadData,
      userPrompt,
      () => {},
      (text) => {
  ...
  renderResult(text, formHtmlBackup, payloadData) {
    const isExcelModule = 'word' === 'excel';
    const exportButtonHtml = isExcelModule
      ? `<button class="cyber-button" onclick="window.BukuPembinaanUI.exportExcel()">
           <span>📊</span> Ekspor Excel (.xls)
         </button>`
      : `<button class="cyber-button" onclick="window.BukuPembinaanUI.exportWord()">
      ...
      <div class="cyber-container fade-in" style="height:100%; display:flex; flex-direction:column; padding:10px;">
         <div class="cyber-title" style="display:flex; justify-content:space-between; align-items:center; border-bottom: 2px solid rgba(0,229,255,0.2); padding-bottom: 15px; margin-bottom: 20px;">
             <div style="display:flex; align-items:center; gap:12px;">
                 <div style="font-size:24px; background:rgba(0,229,255,0.1); width:45px; height:45px; display:flex; align-items:center; justify-content:center; border-radius:10px; border:1px solid rgba(0,229,255,0.3); box-shadow: 0 0 15px rgba(0,229,255,0.1)">✨</div>
                 <div style="display:flex; flex-direction:column;">
                     <span style="font-size:18px; font-weight:700; letter-spacing:1px; color:#fff; text-shadow: 0 0 10px rgba(0,229,255,0.3); font-family:'Plus Jakarta Sans', sans-serif;">PREVIEW EDITOR: ${'Buku Pembinaan'}</span>
         ...
         <div style="flex:1; display:flex; gap:20px; overflow:hidden; margin-bottom:15px;">
            <div style="flex:1; display:flex; flex-direction:column; background:rgba(255,255,255,0.01); border:1px solid rgba(0,229,255,0.15); border-radius:8px; padding:12px;">
               ...
               <textarea id="ai-result-editor" class="cyber-input" style="flex:1; width:100%; height:100%; font-family: 'Courier New', Courier, monospace; font-size:13px; line-height:1.6; resize:none; background:rgba(0,0,0,0.35); border-color:rgba(0,229,255,0.2); color:#e0f0ff;" oninput="window.BukuPembinaanUI.updatePreview()">${text.trim()}</textarea>
            </div>
         ...
         <div style="padding-top:20px; border-top:1px solid rgba(0,229,255,0.15); display:flex; gap:15px; justify-content:space-between; align-items:center;">
             <button class="cyber-button" onclick="window.BukuPembinaanUI.restoreForm()">
                 <span>←</span> Edit Parameter
             </button>
             <div style="display:flex; gap:10px;">
                 <button class="cyber-button" onclick="window.BukuPembinaanUI.saveFinalDocument()">
                     <span>💾</span> Simpan Dokumen
                 </button>
                 ${exportButtonHtml}
                 <button class="cyber-button primary" onclick="window.BukuPembinaanUI.printDocument()">
  ...
  updatePreview() {
    ...
    const schoolProfile = window.BukuPembinaanCore.schoolProfile;
    const kop = `
  ...
  async saveFinalDocument() {
    ...
    if (!editor) return;
    try {
      await window.BukuPembinaanCore.saveFinalDocument(editor.value);
  ...
  async exportWord() {
    ...
    try {
      ...
      `;
      const cleanFileName = 'Buku_Pembinaan';
