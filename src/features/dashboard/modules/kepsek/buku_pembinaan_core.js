// buku_pembinaan_core.js - Core logic for Buku Pembinaan
window.BukuPembinaanCore = {
  db: null,
  userProfile: null,
  schoolProfile: null,
  lastResult: null,
  ...
  async executeAI(payloadData, userPrompt, onStart, onComplete, onError) {
    ...
    try {
      ...
      let systemPrompt = '';
      if ('k_buku_pembinaan'.startsWith('g_')) {
      ...
      } else if ('k_buku_pembinaan'.startsWith('k_')) {
      ...
      } else if ('k_buku_pembinaan'.startsWith('t_')) {
      ...
      } else if ('k_buku_pembinaan'.startsWith('b_')) {
...
Susunlah dokumen ini secara lengkap, detil, tanpa teks pemotong, siap pakai, dan dalam format Markdown lengkap dengan struktur penulisan resmi.`;
      ...
      console.error('[BukuPembinaanCore] AI Error:', err);
      onError(err);
    }
  },
  ...
  async saveDraft(payloadData, userPrompt) {
    ...
    const docId = `${this.userProfile.uid || 'guest'}_k_buku_pembinaan`;
    await this.db.collection('administrasi_drafts').doc(docId).set({
      modId: 'k_buku_pembinaan',
      title: 'Buku Pembinaan',
